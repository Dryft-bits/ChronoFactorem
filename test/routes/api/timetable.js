...
describe("TimeTable api tests", function () {
  ...
  it("/save does not allow malformed requests 2", async function () {
    ...
    requestData = JSON.parse('{"id":null,"name":"timie timie"}');
    server.request.user = testUser;
    chai
      .request(server)
      .post("/api/timetable/save")
